    ...
    <ul class="list-disc space-y-2">
        @forelse ($categories as $category)
            <li class="p-4 bg-gray-100 rounded hover:bg-gray-200 shadow flex justify-between items-center">
                <div>
                    <a href="{{ route('admin.categories.show', $category->slug) }}" class="text-blue-500 hover:underline font-bold block mb-2">
                        {{ $category->name }}
                    </a>
                </div>
                <div>
                    <a href="{{ route('admin.categories.show', $category->slug) }}" class="text-green-500 hover:text-green-700 font-bold mr-2">Show</a>
                    <a href="{{ route('admin.categories.edit', $category->slug) }}" class="text-yellow-500 hover:text-yellow-700 font-bold mr-2">Update</a>
